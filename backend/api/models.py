...
class Chat(models.Model):
    ...
    id = models.CharField(max_length=100, primary_key=True, db_index=True)  # Fixed: added primary_key=True
    ...
    user1 = models.ForeignKey(User, on_delete=models.CASCADE, related_name='chats_as_user1', null=True, blank=True)
    user2 = models.ForeignKey(User, on_delete=models.CASCADE, related_name='chats_as_user2', null=True, blank=True)
    
    # For group chats, store member IDs as JSON
    members = models.JSONField(default=list, blank=True)
    
    # Chat metadata
    last_message = models.ForeignKey('Message', on_delete=models.SET_NULL, null=True, blank=True, related_name='chat_last_message')
    last_activity = models.DateTimeField(auto_now=True)
    is_active = models.BooleanField(default=True)
    
    # Group chat specific fields
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_chats', null=True, blank=True)
...
class Message(models.Model):
    ...
    # Message metadata
    status = models.CharField(max_length=10, choices=MESSAGE_STATUS_CHOICES, default=MESSAGE_STATUS_SENT)
    ...
    def __str__(self) -> str:
        return f"[{self.chat_type}:{self.chat_id}] {self.sender_name}: {self.content[:30]}"
...
class SystemMessage(models.Model):
    ...
    recipient_id = models.CharField(max_length=100)  # Using CharField instead of ForeignKey
...
class Update(models.Model):
    ...
    type = models.CharField(max_length=20, choices=UPDATE_TYPE_CHOICES, default=UPDATE_TYPE_NEWS)
    title = models.CharField(max_length=255)
    summary = models.TextField()
    body = models.TextField()
    timestamp = models.DateTimeField(default=timezone.now)
    status = models.CharField(max_length=20, choices=UPDATE_STATUS_CHOICES, default=UPDATE_STATUS_NEW)
...
class UpdateRelation(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    source_update = models.ForeignKey(Update, on_delete=models.CASCADE, related_name='source_relations')
    target_update = models.ForeignKey(Update, on_delete=models.CASCADE, related_name='target_relations')
    relation_type = models.CharField(max_length=50, default='related')  # related, follows, references, etc.
    created_at = models.DateTimeField(auto_now_add=True)
...
class UpdateLike(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='update_likes')
    is_like = models.BooleanField(default=True)  # True for like, False for dislike
    created_at = models.DateTimeField(auto_now_add=True)
    
    class Meta:
        unique_together = ['update', 'user']  # One user can only like/dislike once per update
        ordering = ['-created_at']
