...
def jwt_login_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        print(
            f"DEBUG: jwt_login_required - Is AnonymousUser: {isinstance(request.user, AnonymousUser)}"
        )
        print(
            f"DEBUG: jwt_login_required - Auth header: {request.META.get('HTTP_AUTHORIZATION', 'None')}"
        )

        # Check if user is authenticated (either via JWT or session)
        if request.user and not isinstance(request.user, AnonymousUser):
            print(
                f"DEBUG: jwt_login_required - User authenticated: {request.user.username}"
            )
