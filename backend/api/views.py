...
from .models import Message, Chat, SystemMessage, Update, UpdateAttachment, UpdateMedia, UpdateLike, UpdateComment, Tag
...
def user_info_view(request: HttpRequest) -> JsonResponse:
    ...
    if request.method == 'GET':
        ...
        print(f"DEBUG: user_info_view - Auth header: {request.META.get('HTTP_AUTHORIZATION', 'None')}")
    ...
    try:
        ...
        if request.method == 'GET':
            try:
                ...
                messages = Message.objects.filter(chat=chat).order_by('-timestamp')[start:end]
            ...
            try:
                ...
                return JsonResponse({
                    ...
                    'message': {
                        ...
                        'sender_id': getattr(getattr(message, 'sender', None), 'id', None) if getattr(message, 'sender', None) else None,
                        'sender_name': (
                            getattr(getattr(message, 'sender', None), 'get_full_name', lambda: None)() or
                            getattr(getattr(message, 'sender', None), 'username', None)
                        ) if getattr(message, 'sender', None) else None,
        ...
        try:
            ...
            try:
                ...
                if chat.chat_type == 'user':
                    ...
                    if not chat.members.filter(id=getattr(request.user, 'id', None)).exists():
                        return JsonResponse({
                            'success': False,
                            'error': 'Access denied'
                        }, status=403)
                ...
                for msg in messages:
                    result = {
                        ...
                        'sender_name': (
                            getattr(getattr(msg, 'sender', None), 'get_full_name', lambda: None)() or
                            getattr(getattr(msg, 'sender', None), 'username', None)
                        ) if getattr(msg, 'sender', None) else None,
                ...
                update_data = {
                    ...
                    'content': {
                        ...
                        'attachments': [
                            ...
                            } for att in update.get_attachments() if hasattr(update, 'get_attachments')
                        ],
                        'media': [
                            {
                            ...
                            } for med in update.get_media() if hasattr(update, 'get_media')
                        ],
...
def comment_list_view(request: HttpRequest, update_id: str) -> JsonResponse:
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            try:
                comments = update.comments.filter(  # type: ignore
                    is_active=True,
            ...
            for comment in comments:
                comment_data = {
                    ...
                    'author': comment.author.username,
                    'created_at': comment.created_at.isoformat(),
                    'can_edit': comment.can_edit(request.user) if hasattr(request, 'user') and request.user.is_authenticated else False,
                    'can_delete': comment.can_delete(request.user) if hasattr(request, 'user') and request.user.is_authenticated else False,
                    'replies': []
                }
                ...
                for reply in comment.replies.filter(is_active=True):
                    reply_data = {
                        ...
                        'can_edit': reply.can_edit(request.user) if hasattr(request, 'user') and request.user.is_authenticated else False,
                        'can_delete': reply.can_delete(request.user) if hasattr(request, 'user') and request.user.is_authenticated else False,
                    }
