...
from .models import Activity, ActivityPriority, ActivityStatus, Update
...
class Query(graphene.ObjectType):
    ...
    activities_by_type = graphene.List(
        ActivityType, type=graphene.String(required=True)
    )
    activities_by_status = graphene.List(
        ActivityType, status=graphene.String(required=True)
    )
    activities_by_priority = graphene.List(
        ActivityType, priority=graphene.String(required=True)
    )
    ...
    news_updates = graphene.List(
        UpdateType, limit=graphene.Int(), offset=graphene.Int()
    )
    ...
    def resolve_news_updates(self, info, limit=None, offset=None, **kwargs):
        ...
        queryset = (
            ...
            .filter(type=Update.UPDATE_TYPE_NEWS, is_active=True)
