  ...
  static async getTags(
    ...
    try {
      ...
      try {
        ...
        console.debug(
          "No authentication available, proceeding without auth headers",
        );
      }
...
export class UpdateApiService {
        ...
        return {
          updates: data.updates.map(
            (update: any): UpdateExtended => ({
              ...
              isExpired: update.expires_at
                ? new Date(update.expires_at) < new Date()
                : false,
              timeAgo: update.created_at
                ? `${Math.floor((Date.now() - new Date(update.created_at).getTime()) / 60000)} min ago`
                : "",
              readingTime: Math.max(
                1,
                Math.ceil((update.content.body || "").split(" ").length / 200),
              ),
              fullName: update.created_by
                ? `${update.created_by.firstName} ${update.created_by.lastName}`.trim()
                : update.author || "",
  ...
  static async updateUpdateStatus(
    updateId: string,
    status: string,
  ): Promise<{ id: string; status: string }> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/updates/${updateId}/status/`,
        {
          method: "PUT",
          headers: AuthService.getAuthHeaders(),
          credentials: "include",
  ...
  static async searchUpdates(
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/updates/search/?${params}`,
        {
          method: "GET",
          headers: AuthService.getAuthHeaders(),
          credentials: "include",
  ...
  static async getUpdatesEnhanced(
    params: {
      type?: string;
      status?: string;
      searchQuery?: string;
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/updates/?${queryParams}`,
        {
          headers: AuthService.getAuthHeaders(),
        },
      );
  ...
  static async editUpdateNew(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/updates/${updateId}/edit/`,
        {
          method: "PUT",
          headers: {
            ...AuthService.getAuthHeaders(),
  ...
  static async deleteUpdateNew(updateId: string): Promise<any> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/updates/${updateId}/delete/`,
        {
          method: "DELETE",
          headers: AuthService.getAuthHeaders(),
        },
  ...
  static async toggleLike(updateId: string, isLike: boolean): Promise<any> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/updates/${updateId}/like/`,
        {
          method: "POST",
          headers: {
            ...AuthService.getAuthHeaders(),
  ...
  static async getComments(updateId: string): Promise<any> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/updates/${updateId}/comments/`,
        {
          headers: AuthService.getAuthHeaders(),
        },
      );
  ...
  static async createComment(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/updates/${updateId}/comments/create/`,
        {
          method: "POST",
          headers: {
            ...AuthService.getAuthHeaders(),
  ...
  static async editComment(commentId: number, content: string): Promise<any> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/comments/${commentId}/edit/`,
        {
          method: "PUT",
          headers: {
            ...AuthService.getAuthHeaders(),
