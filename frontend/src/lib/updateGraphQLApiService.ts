...
export function initializeGraphQLClient(apolloClient: ApolloClient<NormalizedCacheObject>) {
  client = apolloClient;
}

export class UpdateGraphQLApiService {
  static async getUpdates(
    // Parameters like type, status, etc. will be handled by client-side filtering for now,
    ...
    try {
      ...
      const processedUpdates = (data.allUpdates || []).map((rawUpdate: any): UpdateExtended => ({
        id: rawUpdate.id,
        ...
        readingTime: Math.max(1, Math.ceil((rawUpdate.body || '').split(' ').length / 200)),
        fullName: rawUpdate.createdBy ? `${rawUpdate.createdBy.firstName} ${rawUpdate.createdBy.lastName}`.trim() : rawUpdate.author || '',
