...
export const NewsProvider = ({
  ...
  console.log('NewsProvider rendering with props:', { initialFilters, initialPagination });

  // ===== State =====
  const [selectedUpdate, setSelectedUpdate] = useState<UpdateExtended | null>(null);
  const [filters, setFiltersState] = useState<UpdateFilters>(initialFilters);
  const [pagination, setPaginationState] = useState<UpdatePagination>(initialPagination);
  ...
  if (error && !data) {
    console.error('NewsProvider critical error, providing fallback context:', error);
  }

  // ===== Data Processing =====
  // Transform raw GraphQL data to our extended format
  const processedUpdates: UpdateExtended[] = useMemo(() => {
    console.log('Processing updates from raw data:', data?.allUpdates?.length || 0);
    const processed = (data?.allUpdates || []).map((rawUpdate: any) => ({
      ...
      isExpired: rawUpdate.expiresAt ? new Date(rawUpdate.expiresAt) < new Date() : false,
      timeAgo: rawUpdate.createdAt ? Math.floor((Date.now() - new Date(rawUpdate.createdAt).getTime()) / 60000) + ' min ago' : '',
      readingTime: Math.max(1, Math.ceil((rawUpdate.body || '').split(' ').length / 200)), // Approx reading time
    ...
    console.log('Processed update types:', processed.map((u: UpdateExtended) => u.type));
    return processed;
  }, [data?.allUpdates]);
  ...
  const filteredUpdates = useMemo(() => {
    ...
    console.log('Update types:', processedUpdates.map(u => u.type));
    ...
    if (filters.status) {
      ...
      console.log(`Status filter: ${beforeCount} -> ${filtered.length} updates`);
    }
    ...
    if (filters.activeOnly) {
      ...
      console.log(`Active filter: ${beforeCount} -> ${filtered.length} updates`);
    }
    ...
    if (filters.searchQuery) {
      ...
      console.log(`Search filter: ${beforeCount} -> ${filtered.length} updates`);
    }
  ...
  const convertToLegacyUpdate = useCallback((update: UpdateExtended): Update => ({
    id: update.id,
    type: update.type,
    title: update.title,
  ...
  const setPagination = useCallback((newPagination: Partial<UpdatePagination>) => {
  ...
  const filterUpdates = useCallback(async (type: string, status: string, searchQuery: string) => {
  ...
  // ===== Filtering Helpers =====
  const getUpdatesByType = useCallback((type: string): UpdateExtended[] => {
    return filteredUpdates.filter(update => update.type === type);
  }, [filteredUpdates]);

  const getUpdatesByStatus = useCallback((status: string): UpdateExtended[] => {
    return filteredUpdates.filter(update => update.status === status);
  }, [filteredUpdates]);

  const searchUpdates = useCallback((query: string): UpdateExtended[] => {
  ...
  const toggleLike = useCallback(async (_updateId: string, _isLike: boolean) => {
    console.warn('toggleLike not implemented in new context');
  }, []);

  const createComment = useCallback(async (_updateId: string, _content: string, _parentCommentId?: number) => {
    console.warn('createComment not implemented in new context');
  }, []);

  const editComment = useCallback(async (_commentId: number, _content: string) => {
  ...
  const editUpdate = useCallback(async (_updateId: string, _updateData: any) => {
