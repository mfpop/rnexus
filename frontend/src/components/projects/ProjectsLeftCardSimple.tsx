...
const ProjectsLeftCardSimple: React.FC = () => {
  ...
  const { currentPage, goToPage, setRecordsPerPage, updatePagination } =
    usePagination();
  ...
  useEffect(() => {
    updatePagination({
      ...
      totalPages: Math.max(
        1,
        Math.ceil(sortedAndFilteredProjects.length / recordsPerPage),
      ),
    });
  ...
  const handleSortChange = (
    newSortBy: "name" | "progress" | "priority" | "deadline",
  ) => {
    setSortBy(newSortBy);
    setShowSortDropdown(false);
  };
      ...
      <div className="flex border-b border-gray-200 overflow-x-auto bg-gray-50">
        {[
          { key: "all", label: "All", count: sortedAndFilteredProjects.length },
          {
            key: "active",
            label: "Active",
            count: sortedAndFilteredProjects.filter(
              (p) => p.status === "active",
            ).length,
          },
          {
