...
export const ProfileAutosaveProvider: React.FC<ProfileAutosaveProviderProps> = ({ children }) => {
  const [updateUserProfile, { loading: isAutosaving }] = useMutation(UPDATE_USER_PROFILE);
  ...
  const autosaveField = useCallback(async (field: string, value: any) => {
    try {
      ...
      if (!result.data?.updateUserProfile.ok) {
        console.error(`Autosave failed for ${field}:`, result.data?.updateUserProfile.errors);
      }
    } catch (error) {
