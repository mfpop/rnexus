...
const DailyActivitiesCard: React.FC<DailyActivitiesCardProps> = ({
  ...
  const getFilteredActivities = (): Activity[] => {
    ...
    let filtered = data.allActivities.filter((activity: Activity) => {
      ...
      if (assignedTo && !activity.assignedTo.toLowerCase().includes(assignedTo.toLowerCase())) {
        return false;
      }

      // Enhanced filtering by position/department keywords
      if (position || department) {
        const positionKeywords = position ? position.toLowerCase().split(/\s+/) : [];
        const departmentKeywords = department ? department.toLowerCase().split(/\s+/) : [];
        const allKeywords = [...positionKeywords, ...departmentKeywords];

        const activityText = `${activity.title} ${activity.description} ${activity.type} ${activity.location || ''}`.toLowerCase();

        // Check if any keyword matches
        const hasKeywordMatch = allKeywords.some(keyword =>
          keyword.length > 2 && activityText.includes(keyword)
        );

        // Also check for common role-related terms
        const roleTerms = ['manager', 'director', 'supervisor', 'lead', 'senior', 'junior', 'assistant', 'coordinator', 'specialist', 'analyst', 'engineer', 'developer', 'designer'];
        const hasRoleMatch = roleTerms.some(term =>
          (position && position.toLowerCase().includes(term) && activityText.includes(term)) ||
          (department && department.toLowerCase().includes(term) && activityText.includes(term))
        );
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4">
        {(() => {
          const displayActivities = timeRange === 'today' ? todayActivities : thisWeekActivities;
          const timeLabel = timeRange === 'today' ? 'today' : 'this week';

          return displayActivities.length === 0 ? (
            <div className="text-center py-8">
              <Calendar className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <h4 className="text-lg font-medium text-gray-900 mb-2">No activities {timeLabel}</h4>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      ...
                      <div className="flex items-center gap-1">
                        <span>Duration: {formatDuration(activity.estimatedDuration)}</span>
                      </div>
                      {activity.location && (
                        <div className="flex items-center gap-1">
        ...
        return displayActivities.length > 0 ? (
          <div className="p-4 border-t border-gray-200 bg-gray-50">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-medium text-gray-700">Summary for {timeLabel}</h4>
            ...
            <div className="grid grid-cols-4 gap-4 text-center">
              ...
              <div>
                <div className="text-2xl font-bold text-red-600">
                  {displayActivities.filter(a => a.status === 'overdue').length}
                </div>
