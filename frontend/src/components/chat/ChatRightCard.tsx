...
const ChatRightCard: React.FC = () => {
  const { selectedContact, setSelectedContact, showProfileView, setShowProfileView } = useChatContext();
  const { user: currentUser } = useAuth();

  // Debug profile view state
  console.log("ChatRightCard - showProfileView:", showProfileView, "selectedContact:", selectedContact?.name);
  ...
  const [messageOptionsOpen, setMessageOptionsOpen] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    if (graphqlMessages.length > 0) {
      console.log(
        "🔄 Syncing GraphQL messages to local state:",
  ...
  const handleForward = useCallback((message: Message) => {
    ...
    const contactOptions = contacts
      .map((contact, index) => `${index + 1}. ${contact}`)
      .join("\n");

    const selectedContact = prompt(
    ...
    if (selectedContact && !isNaN(Number(selectedContact))) {
      ...
      if (contactIndex >= 0 && contactIndex < contacts.length) {
        alert(
          `✅ Message forwarded to: ${contacts[contactIndex]}\n\nOriginal: "${message.content}"`,
        );
      } else {
  ...
  const handleBulkForward = useCallback(() => {
    ...
    const selectedContact = prompt(
      `Forward ${selectedMessages.size} messages to:\n\n${contactOptions}\n\nEnter contact number (1-${contacts.length}):`,
    );
    ...
    if (selectedContact && !isNaN(Number(selectedContact))) {
      ...
      if (contactIndex >= 0 && contactIndex < contacts.length) {
        alert(
          `✅ ${selectedMessages.size} messages forwarded to: ${contacts[contactIndex]}`,
        );
        exitSelectionMode();
      } else {
        alert("❌ Invalid contact selection");
    ...
    return (
      <div className="h-full flex items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div
            className="absolute -bottom-24 -left-24 w-40 h-40 bg-gradient-to-br from-green-200 to-emerald-200 rounded-full opacity-40 animate-pulse"
            style={{ animationDelay: "1s" }}
          ></div>
          <div
            className="absolute top-1/3 left-1/5 w-28 h-28 bg-gradient-to-br from-purple-200 to-violet-200 rounded-full opacity-30 animate-pulse"
            style={{ animationDelay: "2s" }}
          ></div>

          {/* Secondary accent elements */}
          <div
            className="absolute top-1/4 right-1/4 w-16 h-16 bg-gradient-to-br from-pink-200 to-rose-200 rounded-full opacity-25 animate-pulse"
            style={{ animationDelay: "0.5s" }}
          ></div>
          <div
            className="absolute bottom-1/3 right-1/3 w-20 h-20 bg-gradient-to-br from-yellow-200 to-orange-200 rounded-full opacity-25 animate-pulse"
              ...
              <div className="w-8 h-8 bg-gradient-to-br from-green-100 to-green-200 rounded-lg flex items-center justify-center mb-2 mx-auto group-hover:scale-110 transition-transform duration-300">
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M15.536 11.293l-2.829-2.829a1 1 0 00-1.414 0L8.464 11.293a1 1 0 000 1.414l2.829 2.829a1 1 0 001.414 0l2.829-2.829a1 1 0 000-1.414z"
                  />
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M11 17a1 1 0 102 0m0 0V9a5 5 0 10-5 5v3a1 1 0 102 0z"
                  />
                </svg>
              </div>
              <p className="text-xs font-semibold text-gray-700">
