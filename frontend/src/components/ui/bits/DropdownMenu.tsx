...
>(({ className, children, align = "start", ...props }, ref) => {
  ...
  const updatePosition = useCallback(() => {
    ...
    rafRef.current = window.requestAnimationFrame(() => {
      ...
      // Force a reflow to ensure content dimensions are calculated
      ...
      // Enhanced boundary checks with better edge case handling
      const padding = 8;
      ...
      // Vertical boundary checks - prefer opening below, but flip above if needed
      const spaceAbove = triggerRect.top;

      if (top + contentRect.height > windowHeight - padding) {
        // Not enough space below, try opening above
        if (spaceAbove >= contentRect.height + padding) {
  ...
  useEffect(() => {
    if (context?.open) {
      // Multiple position updates to handle dynamic content sizing
  ...
  const dropdownContent = (
    <div
      ...
      className={cn(
        "fixed z-[99999] min-w-40 bg-white border border-gray-200 rounded-lg shadow-lg py-1 pointer-events-auto",
        className,
      )}
  ...
  // Use portal to render outside of any potentially clipping containers
