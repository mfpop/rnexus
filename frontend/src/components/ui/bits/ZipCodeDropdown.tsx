...
import LocalAddressService, {
  AddressSuggestion,
} from "../../../lib/localAddressService";
...
export const ZipCodeDropdown: React.FC<ZipCodeDropdownProps> = ({
  ...
  const loadZipCodeSuggestions = async () => {
    ...
    try {
      ...
      if (city && state) {
        // Get ZIP codes from local data
        zipCodes = await LocalAddressService.getZipCodes(city, state, country);

        // If no results from local data, use fallback data
        if (zipCodes.length === 0) {
          console.log(
            `No ZIP codes found in local data for ${city}, ${state}, using fallback data`,
          );
          zipCodes = LocalAddressService.getFallbackZipCodes(city, state);
        }
      } else if (state) {
        // If only state is available, get all ZIP codes for the state
        zipCodes = LocalAddressService.getFallbackZipCodes("", state);
      } else {
      ...
      console.error(
        "Failed to load ZIP codes from local data, using fallback data:",
        error,
      );
      // Use fallback data on error
      const fallbackZipCodes = LocalAddressService.getFallbackZipCodes(
        city || "",
        state || "",
      );
