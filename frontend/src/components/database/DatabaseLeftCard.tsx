...
import { Database, Server, Shield, Settings, Monitor, AlertCircle } from "lucide-react";
    ...
    <div className="h-full bg-white flex flex-col">
          ...
          {databaseFeatures.map((feature, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                ...
                feature.status === 'Normal' ? 'bg-blue-100 text-blue-800' :
                feature.status === 'Secure' ? 'bg-purple-100 text-purple-800' :
          ...
          <button className="w-full text-left p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600">Verify database connectivity</p>
              </div>
            </div>
          </button>
      ...
      <div className="p-6 border-t border-gray-100">
        <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
          ...
          <div>
            <p className="text-xs font-medium text-amber-800">Administrator Access</p>
            <p className="text-xs text-amber-700">Database modifications require admin privileges</p>
          </div>
        </div>
      </div>
    </div>
