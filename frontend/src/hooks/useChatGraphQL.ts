...
export const useChatGraphQL = (): UseChatGraphQLReturn => {
  ...
  const [
    loadUserChatsQuery,
    { loading: chatsLoading, data: chatsData, error: chatsError },
  ] = useLazyQuery(GET_USER_CHATS);
  ...
  useEffect(() => {
    if (
      sendMessageData?.createMessage?.ok &&
      sendMessageData?.createMessage?.message
    ) {
      ...
      setChats((prev) =>
        prev.map((chat) =>
          chat.id === sendMessageData.createMessage.message.chatId
            ? {
                ...chat,
                lastMessage: sendMessageData.createMessage.message,
  ...
  const loadMessages = useCallback(
    async (chatId: string, chatType: string) => {
      try {
        setError(null);
  ...
  const createChat = useCallback(
    async (chatData: CreateChatInput): Promise<Chat | null> => {
      try {
        setError(null);
  ...
  const sendMessage = useCallback(
    async (messageData: SendMessageInput): Promise<Message | null> => {
      try {
        setError(null);
        ...
        if (
          result.data?.createMessage?.ok &&
          result.data?.createMessage?.message
        ) {
