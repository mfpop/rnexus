...
test('create activity end-to-end', async ({ page }) => {
  ...
  await expect(page.getByPlaceholder('Enter activity title')).toBeVisible({ timeout: 5000 });
  await page.fill('input[placeholder="Enter activity title"]', 'E2E Test Activity');
  await page.fill('textarea[placeholder="Describe the activity"]', 'Created by automated test');

  // Submit
  await page.getByTestId('create-activity-modal').getByRole('button', { name: 'Create Activity' }).click();

  // Wait for modal to close
  await expect(page.getByTestId('create-activity-modal')).toHaveCount(0, { timeout: 5000 });
